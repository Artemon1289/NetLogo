        ...
        case versionRegex(majorText, minorText, patchText, standardModifier(modifier, modifierNum)) =>
          ...
          (major, minor, patch) match {
            case (m, 0, 0) => baseVersion - 10000 +
              (modifierNum.toInt - 1) + (if (modifier == "RC" || modifier == "BETA" || modifier == "INTERNAL" || modifier == "internal") 5000 else 0)
            case (m, n, 0) => baseVersion - 200 +
              (modifierNum.toInt - 1) + (if (modifier == "RC" || modifier == "BETA" || modifier == "INTERNAL" || modifier == "internal") 100 else 0)
            case (m, n, p) => baseVersion - 10 +
              (modifierNum.toInt) + (if (modifier == "RC" || modifier == "BETA"|| modifier == "INTERNAL" || modifier == "internal") 5 else 0)
          }
